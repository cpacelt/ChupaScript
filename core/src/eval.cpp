...
/// Лежит ли на верхнем уровне агрегата другой агрегат?
///
/// Вопрос задаётся только про аргумент ХОСТ-вызова и только на пути промаха.
/// Билтинов это не касается: все двенадцать читают исключительно верхний
/// уровень своего аргумента (core/src/builtin.cpp), и записи самой коробки им
/// довольно.
///
/// Хост так себя не ограничен. Получив ручку, он вправе спуститься по ней
/// сколь угодно глубоко своими chupa_object_get и chupa_array_at — а эпохи
/// вверх по дереву не всплывают: arrayPush поднимает эпоху СВОЕЙ коробки и
/// ничьей больше. Значит total(state), суммирующая state.items, не встречает
/// push(state.items, x) ни в одной зависимости: спуск движка дошёл только до
/// state, аргументом связан тоже state. Ложное попадание.
///
/// Правило точное и одностороннее:
///   - спуститься ниже хост может только через вложенную ручку, а всякая
///     такая ручка видна этой проверкой в момент захвата;
///   - если агрегат обзаведётся вложенным позже, его собственная эпоха при
///     этом поднимется, читатель промахнётся, и на пересчёте проверка увидит
///     вложенное и выключит кэш;
///   - плоский агрегат из скаляров остаётся кэшируемым, и его собственная
///     эпоха ловит любую правку элементов.
///
/// Цена — обход верхнего уровня агрегата-аргумента, только у хост-вызовов и
/// только на промахе, где мы и так платим за вычисление. Вглубь обход не
/// идёт: первого встреченного агрегата достаточно, чтобы ответить.
///
/// Отвергнутая альтернатива — оговорка в заголовке «не читайте ниже первого
/// уровня аргумента». В этом дереве так не делается: инкремент эпохи живёт
/// внутри мутатора именно затем, чтобы мутация была невыразима мимо подъёма,
/// и проверяется это грепом, а не дисциплиной. «Не читайте часы» хост видит
/// глазом, «не читайте ниже первого уровня» он нарушит случайно — и цена
/// нарушения не лишний пересчёт, а отгруженный застывший экран.
bool holdsNestedAggregate(const Value &v) noexcept {
    const auto isAggregate = [](const Value &element) {
        return element.kind() == Value::Kind::Array ||
               element.kind() == Value::Kind::Object;
    };
    if (v.kind() == Value::Kind::Array) {
        const std::uint32_t count = CS::arrayCount(v);
        for (std::uint32_t i = 0; i < count; ++i) {
            if (isAggregate(CS::arrayAt(v, i))) { return true; }
        }
        return false;
    }
    if (v.kind() == Value::Kind::Object) {
        const std::uint32_t count = CS::objectCount(v);
        for (std::uint32_t i = 0; i < count; ++i) {
            if (isAggregate(CS::objectValueAt(v, i))) { return true; }
        }
        return false;
    }
    return false;
...
bool evalHostCall(const Ast &ast, std::string_view source, NodeId node,
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
        ...
        // А ниже первого уровня набор поручиться уже не может — см.
        // holdsNestedAggregate. Отказ, а не запись: записывать пришлось бы
        // всё поддерево, и потолок в четыре зависимости кончился бы на
        // первом же осмысленном состоянии экрана.
        if (holdsNestedAggregate(argument)) { exec.deps().refuse(); }
        frame[i] = argument;
    }
