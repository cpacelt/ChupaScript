...
class Store {
   public:
    ...
    ///
    /// Мимо ленты записи не существует — это проверяется грепом по шести
    /// дверям (спека §2.2), а не дисциплиной.
    [[nodiscard]] EpochClock &clock() noexcept { return clock_; }

    /// Последний выданный номер, без выдачи нового.
    ///
    /// Отвечает на вопрос «двигалось ли хоть что-нибудь»: мимо ленты записи не
    /// существует, поэтому неизменившийся номер означает, что не записано
    /// ничего. Отдельный метод, а не константная перегрузка `clock()`: часы
    /// наружу отдаются изменяемыми затем, чтобы из них брали номера, и
    /// константная копия того же имени звала бы делать это через const.
    [[nodiscard]] Epoch epochNow() const noexcept { return clock_.now(); }
