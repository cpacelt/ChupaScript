...
// появляется в пользовательском коде часто — каждая обёртка подписывается на
// него (`enum Align: String, CSValue`), — а `Value` без префикса слишком общее
// имя, чтобы занимать его у потребителя.
