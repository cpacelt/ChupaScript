...
for n in [200, 1200, 6000] {
    ...
    let tb = bench("B: классы, без стирания типов", iters: iters) {
        _ = rootB.sizeThatFits(390); rootB.place()
    }
    let tc = bench("C: плоский AoS, циклы", iters: iters) {
        layoutFlat(&nodesC, root: 390)
    }
    print(String(format: "  → A/C %.1fx   A/B %.1fx   B/C %.1fx", ta / tc, ta / tb, tb / tc))
    print(String(format: "  → на узел: A %.1f нс   B %.1f нс   C %.1f нс",
                 ta / Double(real), tb / Double(real), tc / Double(real)))
}
