...
public final class CachedExpression<T: CSCached> {
    ...
    /// Разошёлся ли снимок с эпохами — то есть обязано ли следующее чтение
    /// войти в движок.
    ///
    /// Спрашивают этим не про значение, а про необходимость работы: хост
    /// обходит выражения экрана на каждое уведомление о перерисовке и будит
    /// только те виджеты, у которых что-то действительно сдвинулось. Ответ
    /// стоит четыре загрузки и три сложения и в движок не заходит, поэтому
    /// проход по всем выражениям экрана дешевле одной лишней перерисовки.
    ///
    /// `true` до первого чтения: снимка ещё нет, и сравнивать не с чем.
    /// `true` у некэшируемого выражения всегда: набор зависимостей не годится,
    /// и ответить «свежий» нечем — единственный честный ответ «не знаю»
    /// совпадает здесь с «пересчитай».
    public var isStale: Bool {
        !(hasValue && !uncacheable && sum() == snapshot)
    }
