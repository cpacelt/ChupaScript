...
3. **Выражение с некэшируемым вызовом не кэшируется.** §2.3 обосновывает динамический набор детерминированностью выражений, но хост-функция вправе быть `CHUPA_FN_EFFECT_FREE` и **не** быть `CHUPA_FN_CACHEABLE` — `now()`, `screenWidth()` (`chupascript.h:252`, `docs/semantics.md` §10.1). У `format('${}', now())` набор зависимостей пуст, сумма нулевая, и по §2.6 такое выражение кэшировалось бы навсегда: часы на экране встали бы. Это дыра, а не лишний пересчёт. Закрывается статически, на компиляции. (Исправлено задачей 11: исходный пример здесь был `format('${now()}')` — эта строка `now()` не вызывает вовсе, у `format` позиционные `${}` и аргументы отдельно, `docs/semantics.md` §8.8, а в `'${now()}'` плейсхолдеров ноль.)
...
Третье решение сверх спеки. `format('${}', now())` не зависит ни от одной ячейки и ни от одной коробки: набор пуст, сумма нулевая — и по §2.6 такое выражение кэшировалось бы навсегда. Часы на экране встали бы. Это дыра, а не лишний пересчёт, и закрывается она статически: список вызываемых известен на компиляции. (Исправлено задачей 11: было `format('${now()}')` — литерал без плейсхолдера, `now()` в нём не вызывается; см. правку в Step 1 ниже.)
...
TEST(HostCacheable, AnExpressionOverAClockIsNotCacheable) {
    ...
    //
    // Правка задачей 11: пример ниже был `format('${now()}')`. Это ошибка —
    // `format` подставляет позиционные ${} (docs/semantics.md §8.8), а
    // `${now()}` в шаблоне литерален и `now` не вызывает вовсе, ноль
    // плейсхолдеров. Верно — now() отдельным аргументом:
    CS::Context ctx;
    ChupaFunction desc{};
    desc.name = "now";
    desc.name_len = 3;
