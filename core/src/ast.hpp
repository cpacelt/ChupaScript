...
class Ast {
   public:
    ...
    /// Готовое значение строкового литерала для узла String — координатами в
    /// пуле текста хранилища.
    ///
    /// Байты литерала уложены туда один раз, на компиляции, вместе с
    /// раскодированным экранированием; вычислению остаётся собрать значение
    /// через Store::stringAt — ровно как оно берёт число из узла Number.
    ///
    /// Пара чисел, а не Value: Value шестнадцать байт, а здесь их восемь, и
    ...
    /// координаты отсюда нельзя.
    void stringLiteral(NodeId node, std::uint32_t *offset,
                       std::uint32_t *length) const noexcept;

    /// Записывает координаты уложенного литерала. Зовёт только укладка.
    void setStringLiteral(NodeId node, std::uint32_t offset,
                          std::uint32_t length) noexcept;

    /// Уложен ли литерал этого узла. Пустая строка от неуложенной иначе не
    /// отличается: и та и другая — пара нулей.
   ...
   private:
    ...
    struct Node {
        ...
        union Payload {
            ...
            struct {
                std::uint32_t offset;     ///< смещение в пуле текста
                std::uint32_t length;
            } literal;                    ///< String, после укладки
