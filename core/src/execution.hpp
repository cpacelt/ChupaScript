...
class Execution {
   public:
    ...
    /// они узлы, и живут по счётчику, а не по региону.
    Store scratch;
    ...
    ///
    /// Роль с приходом узлов сузилась до одного случая. Агрегат себя описывает
    /// сам, узел строки тоже — им хранилище не нужно вовсе. Разрешать надо
    /// только промежуточную строку: она смещение, и смысл ему придаёт арена
    /// операции, которая здесь одна.
    ///
    /// У скаляра региона нет, поле у него Counted по умолчанию, и вернётся
