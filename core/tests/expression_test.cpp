...
TEST(Expression, CompilesAndEvaluates) {
    ...
    CS::Diagnostic diag;
    ASSERT_TRUE(expr.eval(store, exec, &out, diag));
...
TEST(Expression, SurvivesBeingMoved) {
    ...
    CS::Value out = CS::Value::null();
    CS::Diagnostic diag;
    ASSERT_TRUE(units[0].eval(store, exec, &out, diag));
    EXPECT_EQ(store.string(out), "Вася");
}
...
TEST(Expression, EvalNumberReturnsOk) {
    ...
    double out = 0.0;
    CS::Diagnostic diag;
    EXPECT_EQ(expr.evalNumber(store, exec, &out, diag), CS::EvalStatus::Ok);
...
TEST(Expression, EvalNumberReturnsNullSeparately) {
    ...
    EXPECT_EQ(expr.evalNumber(store, exec, &out, diag), CS::EvalStatus::Null);
...
TEST(Expression, EvalNumberOnStringIsTypeErrorWithRealOffset) {
    ...
    double out = 42.0;
    CS::Diagnostic diag;
    EXPECT_EQ(expr.evalNumber(store, exec, &out, diag), CS::EvalStatus::Error);
...
TEST(Expression, EvalBoolAndString) {
    ...
    bool b = false;
    EXPECT_EQ(flag.evalBool(store, exec, &b, diag), CS::EvalStatus::Ok);
    EXPECT_TRUE(b);

    CS::Expression text;
    ASSERT_EQ(CS::Expression::compile("'привет'", store, &text, diags, 1), 0u);
    std::string_view s;
    EXPECT_EQ(text.evalString(store, exec, &s, diag), CS::EvalStatus::Ok);
    EXPECT_EQ(s, "привет");
}
...
TEST(Expression, EvalStringPropagatesEvalError) {
    ...
    EXPECT_EQ(expr.evalString(store, exec, &s, diag), CS::EvalStatus::Error);
...
TEST(Expression, StringLiteralIsStoredOnceAtCompileTime) {
    ...
    std::string_view s;
    ASSERT_EQ(expr.evalString(store, exec, &s, diag), CS::EvalStatus::Ok);
