...
|---|---|
| `BM_Eval_FormatNumber` | **заметно быстрее базы**: результат встроенный, аллокации нет |
| `BM_Eval_StringLiteral` | без изменений — литерал уложен на компиляции |
| `BM_Eval_Constant` | без изменений |
| `BM_Eval_ShortPath` | без изменений либо быстрее |
| `BM_Eval_ArrayLiteral` / `ObjectLiteral` | без изменений — их черёд в задаче 9 |

Если `FormatNumber` не вернулся хотя бы к базе — искать, где результат всё ещё
уходит в коробку, до перехода дальше. Записать в
`docs/benchmarks/2026-08-19-memory-model-checkpoint-b.md`.
