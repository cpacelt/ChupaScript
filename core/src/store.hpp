...
class Store {
   public:
    /// Постоянное хранилище контекста: своя таблица имён полей, свой список
    /// отложенного освобождения.
    Store();

    /// Арена операции: таблицы имён нет вовсе, список общий с постоянным
    /// хранилищем — сливаются они всё равно вместе, на одной границе.
    ///
    /// Таблица здесь была дубликатом указателя, удерживаемым ради одного
    /// вызова — makeObject. Тот уехал в aggregate.hpp и берёт таблицу
    /// параметром, а читатели объекта её и так брали не у хранилища, а у самой
    /// коробки (ObjectBox::keys). Держать вторую ссылку стало не для кого.
    explicit Store(Deferred &shared);
    ...
    /// Строка коробкой — единственный способ её получить.
    ///
