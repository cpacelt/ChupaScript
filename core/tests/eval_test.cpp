...
Value evaluate(Store &store, CS::Execution &exec, std::string_view text) {
    Ast ast;
    Diagnostic diag;
...
Diagnostic evalError(Store &store, CS::Execution &exec,
...
/// Хранилище, которому принадлежит значение.
///
/// Результат вычисления живёт во временном регионе, данные хоста — в
/// постоянном, и агрегат вправе смешивать оба: `[user.name]` это временный
/// массив с постоянной строкой внутри (docs/backlog.md [B57]).
const Store &owner(Store &store, CS::Execution &exec, Value v) {
    return CS::storeOf(store, exec, v);
}

/// Вычисляет и читает результат как строку — самая частая пара в этом файле.
std::string_view evalText(Store &store, CS::Execution &exec,
                          std::string_view text) {
    const Value v = evaluate(store, exec, text);
    return owner(store, exec, v).string(v);
}
...
TEST(EvalLiterals, NumberIsEvaluated) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evaluate(store, exec, "3").numberValue(), 3.0);
    EXPECT_EQ(evaluate(store, exec, "0.5").numberValue(), 0.5);
}

TEST(EvalLiterals, BooleanIsEvaluated) {
    Store store;
    CS::Execution exec;
    EXPECT_TRUE(evaluate(store, exec, "true").booleanValue());
    EXPECT_FALSE(evaluate(store, exec, "false").booleanValue());
}

TEST(EvalLiterals, NullIsEvaluated) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evaluate(store, exec, "null").kind(), Value::Kind::Null);
}

TEST(EvalLiterals, StringIsEvaluated) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evalText(store, exec, "'Вася'"), "Вася");
    EXPECT_EQ(evalText(store, exec, "\"Вася\""), "Вася");
}
...
TEST(EvalLiterals, StringEscapesAreDecoded) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evalText(store, exec, "'a\\nb'"), "a\nb");
}
...
TEST(EvalNames, GlobalIsRead) {
    ...
    CS::Execution exec;
    put(store, "count", "3");
    EXPECT_EQ(evaluate(store, exec, "count").numberValue(), 3.0);
}
...
TEST(EvalNames, GlobalHoldingAggregateIsReadByIdentity) {
    ...
    CS::Execution exec;
    put(store, "items", "[1, 2]");
    EXPECT_TRUE(evaluate(store, exec, "items").sameAggregate(store.global("items")));
}
...
TEST(EvalNames, GlobalHoldingNullIsRead) {
    ...
    EXPECT_EQ(evaluate(store, exec, "maybe").kind(), Value::Kind::Null);
}
...
TEST(EvalMember, ExistingKeyIsRead) {
    ...
    EXPECT_EQ(evalText(store, exec, "user.name"), "Вася");
    EXPECT_EQ(evaluate(store, exec, "user.age").numberValue(), 30.0);
}
...
TEST(EvalMember, MissingKeyReadsAsNull) {
    ...
    EXPECT_EQ(evaluate(store, exec, "user.nickname").kind(), Value::Kind::Null);
}
...
TEST(EvalMember, ReadingThroughNullGivesNull) {
    ...
    EXPECT_EQ(evaluate(store, exec, "user.prfoile.avatar").kind(), Value::Kind::Null);
    EXPECT_EQ(evaluate(store, exec, "user.a.b.c.d.e").kind(), Value::Kind::Null);
}
...
TEST(EvalMember, ReadingKeyOffANonObjectIsAnError) {
    ...
    EXPECT_EQ(evalError(store, exec, "count.x").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "name.x").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "flag.x").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "items.x").code, CS::ErrorCode::Type);
}
...
TEST(EvalMember, KeyIsTakenLiterallyNotAsAName) {
    ...
    EXPECT_EQ(evalText(store, exec, "o.name"), "ключ");
}
...
TEST(EvalMember, OffsetPointsAtTheFailingNode) {
    ...
    EXPECT_GT(evalError(store, exec, "count.a.b").offset, 0u);
}
...
TEST(EvalIndex, ArrayElementIsRead) {
    ...
    EXPECT_EQ(evaluate(store, exec, "items[0]").numberValue(), 10.0);
    EXPECT_EQ(evaluate(store, exec, "items[2]").numberValue(), 30.0);
}
...
TEST(EvalIndex, ArrayReadBeyondEndGivesNull) {
    ...
    EXPECT_EQ(evaluate(store, exec, "items[1]").kind(), Value::Kind::Null);
    EXPECT_EQ(evaluate(store, exec, "items[1000000]").kind(), Value::Kind::Null);
}
...
TEST(EvalIndex, FractionalAndNegativeIndicesAreErrors) {
    ...
    EXPECT_EQ(evalError(store, exec, "items[0.5]").code, CS::ErrorCode::Range);
    EXPECT_EQ(evalError(store, exec, "items[minusOne]").code, CS::ErrorCode::Range);
    EXPECT_EQ(evalError(store, exec, "items[huge]").code, CS::ErrorCode::Range);
}
...
TEST(EvalIndex, NonNumberArrayIndexIsAnError) {
    ...
    // не работает.
    EXPECT_EQ(evalError(store, exec, "items['0']").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "items[true]").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "items[null]").code, CS::ErrorCode::Type);
}
...
TEST(EvalIndex, ObjectKeyIsRead) {
    ...
    EXPECT_EQ(evalText(store, exec, "o['name']"), "Вася");
    EXPECT_EQ(evaluate(store, exec, "o['missing']").kind(), Value::Kind::Null);
}
...
TEST(EvalIndex, ScalarKeysAreCoercedToString) {
    ...
    EXPECT_EQ(evalText(store, exec, "o[0]"), "zero");
    EXPECT_EQ(evalText(store, exec, "o[true]"), "yes");
    EXPECT_EQ(evalText(store, exec, "o[null]"), "nothing");
    EXPECT_EQ(evalText(store, exec, "o[1.5]"), "half");
}
...
TEST(EvalIndex, NegativeZeroAndZeroAreDifferentKeys) {
    ...
    EXPECT_EQ(evalText(store, exec, "o[0]"), "plus");
    EXPECT_EQ(evalText(store, exec, "o[minusZero]"), "minus");
}
...
TEST(EvalIndex, AggregateKeyIsAnError) {
    ...
    EXPECT_EQ(evalError(store, exec, "o[items]").code, CS::ErrorCode::Type);
}
...
TEST(EvalIndex, ReadingThroughNullGivesNull) {
    ...
    CS::Execution exec;
    put(store, "user", "{'name': 'Вася'}");
    EXPECT_EQ(evaluate(store, exec, "user.missing[0]").kind(), Value::Kind::Null);
    EXPECT_EQ(evaluate(store, exec, "user.missing['k']").kind(), Value::Kind::Null);
}
...
TEST(EvalIndex, SubscriptIsEvaluatedEvenWhenTheBaseIsNull) {
    ...
    EXPECT_EQ(evalError(store, exec, "user.missing[1 + 'a']").code, CS::ErrorCode::Type);
}
...
TEST(EvalIndex, BaseIsEvaluatedBeforeTheSubscript) {
    ...
    const Diagnostic diag = evalError(store, exec, "(1 + 'a')[2 + 'b']");
...
TEST(EvalIndex, IndexingANonAggregateIsAnError) {
    ...
    EXPECT_EQ(evalError(store, exec, "count[0]").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "name[0]").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "flag[0]").code, CS::ErrorCode::Type);
}
...
TEST(EvalIndex, ChainedAccessWorks) {
    ...
    EXPECT_EQ(evaluate(store, exec, "state.items[1].id").numberValue(), 2.0);
}

TEST(EvalAggregates, ArrayLiteralKeepsOrder) {
    Store store;
    CS::Execution exec;
    const Value a = evaluate(store, exec, "[1, 2, 3]");
    const Store &made = owner(store, exec, a);
...
TEST(EvalAggregates, ObjectLiteralStoresPairs) {
    Store store;
    CS::Execution exec;
    const Value o = evaluate(store, exec, "{'a': 1, 'b': 2}");
    const Store &made = owner(store, exec, o);
...
TEST(EvalAggregates, EmptyLiterals) {
    Store store;
    CS::Execution exec;
    const Value a = evaluate(store, exec, "[]");
    const Value o = evaluate(store, exec, "{}");
    EXPECT_EQ(owner(store, exec, a).arrayCount(a), 0u);
    EXPECT_EQ(owner(store, exec, o).objectCount(o), 0u);
}
...
TEST(EvalAggregates, ElementsAreArbitraryExpressions) {
    ...
    const Value a = evaluate(store, exec, "[user.name, items[0], user.missing]");
    ...
    EXPECT_EQ(owner(store, exec, first).string(first), "Вася");
...
TEST(EvalAggregates, ObjectValuesAreExpressionsAndKeysAreLiterals) {
    ...
    CS::Execution exec;
    put(store, "user", "{'name': 'Вася'}");
    const Value o = evaluate(store, exec, "{'who': user.name}");
    const Value who = owner(store, exec, o).objectGet(o, "who");
    EXPECT_EQ(owner(store, exec, who).string(who), "Вася");
}
...
TEST(EvalDepth, ChainAtTheParserLimitEvaluatesWithoutOverflow) {
    ...
    EXPECT_EQ(evaluate(store, exec, source).kind(), Value::Kind::Null);
...
TEST(EvalDepth, OperatorChainAtTheParserLimitEvaluatesWithoutOverflow) {
    ...
    EXPECT_EQ(evaluate(store, exec, source).numberValue(), 510.0);
...
TEST(EvalAggregates, EachEvaluationCreatesANewAggregate) {
    ...
    EXPECT_EQ(owner(store, exec, first).arrayCount(first), 2u);
    EXPECT_EQ(owner(store, exec, second).arrayCount(second), 2u);
}

TEST(EvalOperators, UnaryWorksThroughTheWalk) {
    Store store;
    CS::Execution exec;
    EXPECT_FALSE(evaluate(store, exec, "!true").booleanValue());
    EXPECT_EQ(evaluate(store, exec, "-3").numberValue(), -3.0);
}
...
TEST(EvalOperators, ArithmeticRespectsPrecedence) {
    ...
    EXPECT_EQ(evaluate(store, exec, "1 + 2 * 3").numberValue(), 7.0);
    EXPECT_EQ(evaluate(store, exec, "(1 + 2) * 3").numberValue(), 9.0);
}

TEST(EvalOperators, ComparisonWorksThroughTheWalk) {
    Store store;
    CS::Execution exec;
    EXPECT_TRUE(evaluate(store, exec, "1 < 2").booleanValue());
    EXPECT_FALSE(evaluate(store, exec, "1 > 2").booleanValue());
}

TEST(EvalOperators, EqualityWorksThroughTheWalk) {
    Store store;
    CS::Execution exec;
    EXPECT_TRUE(evaluate(store, exec, "1 == 1").booleanValue());
    EXPECT_TRUE(evaluate(store, exec, "1 != 2").booleanValue());
    EXPECT_TRUE(evaluate(store, exec, "null == null").booleanValue());
}
...
TEST(EvalOperators, OperandsComeFromTheStore) {
    ...
    EXPECT_EQ(evaluate(store, exec, "state.count + 1").numberValue(), 42.0);
}
...
TEST(EvalOperators, ErrorInTheLeftOperandStopsEvaluation) {
    ...
    EXPECT_EQ(evalError(store, exec, "items[-1] + 1").code, CS::ErrorCode::Range);
}
...
TEST(EvalOperators, ErrorInTheRightOperandStopsEvaluation) {
    ...
    CS::Execution exec;
    put(store, "items", "[1]");
    EXPECT_EQ(evalError(store, exec, "1 + items[-1]").code, CS::ErrorCode::Range);
}
...
TEST(EvalOperators, AggregateEqualityIsByIdentityThroughTheWalk) {
    ...
    EXPECT_TRUE(evaluate(store, exec, "items == items").booleanValue());
    EXPECT_FALSE(evaluate(store, exec, "items == [1, 2]").booleanValue());
}
...
TEST(EvalShortCircuit, AndDoesNotEvaluateTheRightOperand) {
    ...
    EXPECT_FALSE(evaluate(store, exec, "false && (1 + 'a')").booleanValue());
}
...
TEST(EvalShortCircuit, AndEvaluatesTheRightOperandWhenNeeded) {
    ...
    CS::Execution exec;
    put(store, "items", "[1]");
    EXPECT_FALSE(evaluate(store, exec, "true && false").booleanValue());
    EXPECT_TRUE(evaluate(store, exec, "true && true").booleanValue());
    // Range через items[-1]: код, которого && сам не порождает.
    EXPECT_EQ(evalError(store, exec, "true && items[-1]").code, CS::ErrorCode::Range);
}

TEST(EvalShortCircuit, OrDoesNotEvaluateTheRightOperand) {
    Store store;
    CS::Execution exec;
    EXPECT_TRUE(evaluate(store, exec, "true || (1 + 'a')").booleanValue());
}
...
TEST(EvalShortCircuit, OrEvaluatesTheRightOperandWhenNeeded) {
    ...
    CS::Execution exec;
    put(store, "items", "[1]");
    EXPECT_TRUE(evaluate(store, exec, "false || true").booleanValue());
    EXPECT_FALSE(evaluate(store, exec, "false || false").booleanValue());
    // Range через items[-1]: код, которого || сам не порождает.
    EXPECT_EQ(evalError(store, exec, "false || items[-1]").code, CS::ErrorCode::Range);
}
...
TEST(EvalShortCircuit, ErrorOnTheLeftIsNotSwallowed) {
    ...
    EXPECT_EQ(evalError(store, exec, "items[-1] && false").code, CS::ErrorCode::Range);
    EXPECT_EQ(evalError(store, exec, "items[-1] || true").code, CS::ErrorCode::Range);
}
...
TEST(EvalShortCircuit, TypeOfTheUnevaluatedOperandIsNotChecked) {
    ...
    EXPECT_FALSE(evaluate(store, exec, "false && 5").booleanValue());
    EXPECT_EQ(evalError(store, exec, "true && 5").code, CS::ErrorCode::Type);
    // Зеркало для ||: у него замыкает истина, а не ложь.
    EXPECT_TRUE(evaluate(store, exec, "true || 5").booleanValue());
    EXPECT_EQ(evalError(store, exec, "false || 5").code, CS::ErrorCode::Type);
}

TEST(EvalShortCircuit, LogicalOperatorsRequireBooleanOnTheLeft) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evalError(store, exec, "1 && true").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "'a' || true").code, CS::ErrorCode::Type);
}
...
TEST(EvalShortCircuit, GuardIdiomProtectsTheRightSide) {
    ...
    EXPECT_FALSE(evaluate(store, exec, "false && state.items['0'] == 1").booleanValue());
    EXPECT_EQ(evalError(store, exec, "true && state.items['0'] == 1").code,
              CS::ErrorCode::Type);
}
...
TEST(EvalNilCoalesce, TakesTheLeftWhenItIsNotNull) {
    ...
    // null.
    EXPECT_EQ(evaluate(store, exec, "1 ?? (1 + 'a')").numberValue(), 1.0);
}

TEST(EvalNilCoalesce, TakesTheRightWhenTheLeftIsNull) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evaluate(store, exec, "null ?? 2").numberValue(), 2.0);
}
...
TEST(EvalNilCoalesce, DoesNotSwallowErrors) {
    ...
    // не это.
    EXPECT_EQ(evalError(store, exec, "(1 + 'a') ?? 0").code, CS::ErrorCode::Type);
    // И справа тоже: если левый null, правый вычисляется по-настоящему.
    EXPECT_EQ(evalError(store, exec, "null ?? (2 + 'b')").code, CS::ErrorCode::Type);
}

TEST(EvalNilCoalesce, OperandTypesNeedNotMatch) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evalText(store, exec, "null ?? 'запасное'"), "запасное");
}
...
TEST(EvalNilCoalesce, ChainsRightAssociatively) {
    ...
    EXPECT_EQ(evalText(store, exec, "user.nickname ?? user.name ?? 'Гость'"),
              "Гость");
}
...
TEST(EvalTernary, EvaluatesOnlyTheSelectedBranch) {
    ...
    EXPECT_EQ(evaluate(store, exec, "true ? 1 : (1 + 'a')").numberValue(), 1.0);
    EXPECT_EQ(evaluate(store, exec, "false ? (1 + 'a') : 2").numberValue(), 2.0);
}

TEST(EvalTernary, ConditionMustBeBoolean) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evalError(store, exec, "1 ? 1 : 2").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "null ? 1 : 2").code, CS::ErrorCode::Type);
}

TEST(EvalTernary, BranchesNeedNotShareAType) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evaluate(store, exec, "true ? 1 : 'a'").numberValue(), 1.0);
    EXPECT_EQ(evalText(store, exec, "false ? 1 : 'a'"), "a");
}

/// Разбирает и выполняет скрипт; требует успеха обоих шагов.
void run(Store &store, CS::Execution &exec, std::string_view text) {
    Ast ast;
    Diagnostic diag;
...
Diagnostic runError(Store &store, CS::Execution &exec,
                    std::string_view text) {
    Ast ast;
    Diagnostic diag;
    const std::uint32_t errors = CS::compileScript(
...
TEST(EvalAssign, ExistingKeyIsReplaced) {
    ...
    run(store, exec, "state.count = 42;");
    EXPECT_EQ(evaluate(store, exec, "state.count").numberValue(), 42.0);
}
...
TEST(EvalAssign, MissingKeyIsCreated) {
    ...
    run(store, exec, "state.fresh = 'значение';");
    EXPECT_EQ(evalText(store, exec, "state.fresh"), "значение");
}
...
TEST(EvalAssign, ValueMayBeAnyExpression) {
    ...
    run(store, exec, "state.sum = state.a * state.b + 1;");
    EXPECT_EQ(evaluate(store, exec, "state.sum").numberValue(), 7.0);
}
...
TEST(EvalAssign, DeepPathIsWritable) {
    ...
    run(store, exec, "user.profile.city.name = 'Москва';");
    EXPECT_EQ(evalText(store, exec, "user.profile.city.name"), "Москва");
}
...
TEST(EvalAssign, WritingIntoNullIsAnError) {
    ...
    EXPECT_EQ(evaluate(store, exec, "user.profile.name").kind(), Value::Kind::Null);
    EXPECT_EQ(runError(store, exec, "user.profile.name = 'Вася';").code,
              CS::ErrorCode::Type);
}
...
TEST(EvalAssign, WritingAKeyOffANonObjectIsAnError) {
    ...
    put(store, "count", "3");
    put(store, "items", "[1]");
    EXPECT_EQ(runError(store, exec, "count.x = 1;").code, CS::ErrorCode::Type);
    EXPECT_EQ(runError(store, exec, "items.x = 1;").code, CS::ErrorCode::Type);
}
...
TEST(EvalAssign, ErrorInTheValueLeavesTheTargetUntouched) {
    ...
    EXPECT_EQ(runError(store, exec, "state.a = 1 + 'a';").code, CS::ErrorCode::Type);
    EXPECT_EQ(evaluate(store, exec, "state.a").numberValue(), 1.0);
}
...
TEST(EvalAssign, TargetCheckLosesToValueErrorWhenBothFail) {
    ...
    EXPECT_EQ(runError(store, exec, "user.profile.name = items[minusOne];").code,
              CS::ErrorCode::Range);
}
...
TEST(EvalAssignIndex, ArrayElementIsReplaced) {
    ...
    CS::Execution exec;
    put(store, "items", "[10, 20, 30]");
    run(store, exec, "items[1] = 99;");
    EXPECT_EQ(evaluate(store, exec, "items[1]").numberValue(), 99.0);
    EXPECT_EQ(evaluate(store, exec, "items[0]").numberValue(), 10.0);
}
...
TEST(EvalAssignIndex, WritingBeyondTheEndIsAnError) {
    ...
    // обязательны.
    EXPECT_EQ(evaluate(store, exec, "items[1]").kind(), Value::Kind::Null);
    EXPECT_EQ(runError(store, exec, "items[1] = 1;").code, CS::ErrorCode::Range);
    EXPECT_EQ(runError(store, exec, "items[1000000] = 1;").code, CS::ErrorCode::Range);
    // 2^32: приведение к uint32_t усекло бы индекс в ноль, попав в границы.
    EXPECT_EQ(runError(store, exec, "items[4294967296] = 1;").code, CS::ErrorCode::Range);
}
...
TEST(EvalAssignIndex, FractionalAndNegativeIndicesAreErrors) {
    ...
    put(store, "items", "[10, 20]");
    put(store, "minusOne", "-1");
    EXPECT_EQ(runError(store, exec, "items[0.5] = 1;").code, CS::ErrorCode::Range);
    EXPECT_EQ(runError(store, exec, "items[minusOne] = 1;").code, CS::ErrorCode::Range);
}
...
TEST(EvalAssignIndex, NonNumberArrayIndexIsAnError) {
    ...
    CS::Execution exec;
    put(store, "items", "[10, 20]");
    EXPECT_EQ(runError(store, exec, "items['0'] = 1;").code, CS::ErrorCode::Type);
}
...
TEST(EvalAssignIndex, ObjectKeyIsWritten) {
    ...
    CS::Execution exec;
    put(store, "o", "{'a': 1}");
    run(store, exec, "o['a'] = 2;");
    run(store, exec, "o['fresh'] = 3;");
    EXPECT_EQ(evaluate(store, exec, "o.a").numberValue(), 2.0);
    EXPECT_EQ(evaluate(store, exec, "o.fresh").numberValue(), 3.0);
}
...
TEST(EvalAssignIndex, ScalarKeysAreCoercedToString) {
    ...
    run(store, exec, "o[0] = 'ноль';");
    run(store, exec, "o[true] = 'да';");
    run(store, exec, "o[null] = 'ничего';");
    EXPECT_EQ(evalText(store, exec, "o['0']"), "ноль");
    EXPECT_EQ(evalText(store, exec, "o['true']"), "да");
    EXPECT_EQ(evalText(store, exec, "o['null']"), "ничего");
}
...
TEST(EvalAssignIndex, AggregateKeyIsAnError) {
    ...
    put(store, "o", "{}");
    put(store, "items", "[1]");
    EXPECT_EQ(runError(store, exec, "o[items] = 1;").code, CS::ErrorCode::Type);
}
...
TEST(EvalAssignIndex, WritingIntoNullIsAnError) {
    ...
    put(store, "user", "{'name': 'Вася'}");
    EXPECT_EQ(evaluate(store, exec, "user.missing[0]").kind(), Value::Kind::Null);
    EXPECT_EQ(runError(store, exec, "user.missing[0] = 1;").code, CS::ErrorCode::Type);
}
...
TEST(EvalAssignIndex, WritingIntoANonAggregateIsAnError) {
    ...
    put(store, "count", "3");
    put(store, "name", "'Вася'");
    EXPECT_EQ(runError(store, exec, "count[0] = 1;").code, CS::ErrorCode::Type);
    EXPECT_EQ(runError(store, exec, "name[0] = 1;").code, CS::ErrorCode::Type);
}
...
TEST(EvalAssignIndex, ChainedTargetWorks) {
    ...
    run(store, exec, "state.rows[0].cells[1] = 99;");
    EXPECT_EQ(evaluate(store, exec, "state.rows[0].cells[1]").numberValue(), 99.0);
}
...
TEST(EvalAssignIndex, SubscriptMayBeAnExpression) {
    ...
    put(store, "items", "[10, 20, 30]");
    put(store, "i", "1");
    run(store, exec, "items[i + 1] = 99;");
    EXPECT_EQ(evaluate(store, exec, "items[2]").numberValue(), 99.0);
}
...
TEST(EvalCompound, FourOperatorsWorkOnAKey) {
    ...
    CS::Execution exec;
    put(store, "s", "{'n': 10}");
    run(store, exec, "s.n += 5;");
    EXPECT_EQ(evaluate(store, exec, "s.n").numberValue(), 15.0);
    run(store, exec, "s.n -= 3;");
    EXPECT_EQ(evaluate(store, exec, "s.n").numberValue(), 12.0);
    run(store, exec, "s.n *= 2;");
    EXPECT_EQ(evaluate(store, exec, "s.n").numberValue(), 24.0);
    run(store, exec, "s.n /= 4;");
    EXPECT_EQ(evaluate(store, exec, "s.n").numberValue(), 6.0);
}
...
TEST(EvalCompound, WorksOnAnArrayElement) {
    ...
    CS::Execution exec;
    put(store, "items", "[1, 2, 3]");
    run(store, exec, "items[1] += 10;");
    EXPECT_EQ(evaluate(store, exec, "items[1]").numberValue(), 12.0);
}
...
TEST(EvalCompound, TypeMismatchIsAnError) {
    ...
    EXPECT_EQ(runError(store, exec, "s.text += 'б';").code, CS::ErrorCode::Type);
}
...
TEST(EvalCompound, MissingKeyReadsAsNullAndThenFails) {
    ...
    EXPECT_EQ(runError(store, exec, "s.missing += 1;").code, CS::ErrorCode::Type);
}
...
TEST(EvalCompound, BeyondTheEndGivesTypeNotRange) {
    ...
    EXPECT_EQ(runError(store, exec, "items[5] += 1;").code, CS::ErrorCode::Type);
    EXPECT_EQ(runError(store, exec, "items[5] = 1;").code, CS::ErrorCode::Range);
}
...
TEST(EvalCompound, ErrorLeavesTheTargetUntouched) {
    ...
    CS::Execution exec;
    put(store, "s", "{'n': 10}");
    EXPECT_EQ(runError(store, exec, "s.n += 'а';").code, CS::ErrorCode::Type);
    EXPECT_EQ(evaluate(store, exec, "s.n").numberValue(), 10.0);
}
...
TEST(EvalCompound, TargetCheckLosesToValueErrorWhenBothFail) {
    ...
    EXPECT_EQ(runError(store, exec, "items[-1] += 1 + 'a';").code, CS::ErrorCode::Type);
}
...
TEST(EvalCompound, DivisionByZeroFollowsIEEE) {
    ...
    run(store, exec, "s.n /= 0;");
    EXPECT_TRUE(std::isinf(evaluate(store, exec, "s.n").numberValue()));
}
...
TEST(EvalCompound, DeepTargetWorks) {
    ...
    run(store, exec, "state.rows[i].n += 41;");
    EXPECT_EQ(evaluate(store, exec, "state.rows[0].n").numberValue(), 42.0);
}
...
TEST(EvalScriptBehaviour, StatementsApplyInOrder) {
    ...
    run(store, exec, "s.n = s.n + 1; s.n = s.n * 10; s.n = s.n + 3;");
    EXPECT_EQ(evaluate(store, exec, "s.n").numberValue(), 13.0);
}
...
TEST(EvalScriptBehaviour, LaterStatementsSeeEarlierWrites) {
    ...
    CS::Execution exec;
    put(store, "s", "{'a': 1}");
    run(store, exec, "s.b = s.a + 1; s.c = s.b + 1;");
    EXPECT_EQ(evaluate(store, exec, "s.c").numberValue(), 3.0);
}
...
TEST(EvalScriptBehaviour, ErrorStopsTheScriptAndKeepsWhatWasDone) {
    ...
    const Diagnostic diag =
        runError(store, exec, "s.a = 1; s.b = 2; s.x = 1 + 'a'; s.c = 3;");
    EXPECT_EQ(diag.code, CS::ErrorCode::Type);
    EXPECT_EQ(evaluate(store, exec, "s.a").numberValue(), 1.0);
    EXPECT_EQ(evaluate(store, exec, "s.b").numberValue(), 2.0);
    EXPECT_EQ(evaluate(store, exec, "s.c").numberValue(), 0.0);
...
TEST(EvalScriptBehaviour, MutationIsVisibleThroughAnotherName) {
    ...
    run(store, exec, "state.items[0] = 99;");
    EXPECT_EQ(evaluate(store, exec, "shortcut[0]").numberValue(), 99.0);
}
...
TEST(EvalScriptBehaviour, AssignmentCreatesAnAliasJustLikeTheHostDoes) {
    ...
    run(store, exec, "state.b = state.a; state.a[0] = 9;");
    EXPECT_EQ(evaluate(store, exec, "state.b[0]").numberValue(), 9.0);
}
...
TEST(EvalScriptBehaviour, SelfReferenceIsAValidScript) {
    ...
    run(store, exec, "obj['self'] = obj;");
    EXPECT_TRUE(evaluate(store, exec, "obj['self'] == obj").booleanValue());
    EXPECT_TRUE(evaluate(store, exec, "obj['self']['self']['self'] == obj").booleanValue());
}
...
TEST(EvalScriptBehaviour, EmptyStatementsAreSkipped) {
    ...
    CS::Execution exec;
    put(store, "s", "{'n': 0}");
    run(store, exec, ";; s.n = 1 ;;");
    EXPECT_EQ(evaluate(store, exec, "s.n").numberValue(), 1.0);
}
...
TEST(EvalScriptBehaviour, DeepPathInsideAScript) {
    ...
    run(store, exec, "state.rows[0].cells[0] = 1; state.rows[1].cells[0] = 2;");
    EXPECT_EQ(evaluate(store, exec, "state.rows[0].cells[0]").numberValue(), 1.0);
    EXPECT_EQ(evaluate(store, exec, "state.rows[1].cells[0]").numberValue(), 2.0);
}
...
TEST(EvalCall, CountOfEachKind) {
    ...
    put(store, "o", "{'a': 1, 'b': 2}");
    put(store, "empty", "[]");
    EXPECT_EQ(evaluate(store, exec, "count(items)").numberValue(), 3.0);
    EXPECT_EQ(evaluate(store, exec, "count(o)").numberValue(), 2.0);
    EXPECT_EQ(evaluate(store, exec, "count(empty)").numberValue(), 0.0);
}
...
TEST(EvalCall, CountOfStringCountsBytesNotCharacters) {
    ...
    EXPECT_EQ(evaluate(store, exec, "count('привет')").numberValue(), 12.0);
    EXPECT_EQ(evaluate(store, exec, "count('😀')").numberValue(), 4.0);
    EXPECT_EQ(evaluate(store, exec, "count('abc')").numberValue(), 3.0);
    EXPECT_EQ(evaluate(store, exec, "count('')").numberValue(), 0.0);
}

TEST(EvalCall, CountRejectsScalarsOtherThanString) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evalError(store, exec, "count(1)").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "count(true)").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "count(null)").code, CS::ErrorCode::Type);
}
...
TEST(EvalCall, KeysReturnsEveryKey) {
    ...
    const Value keys = evaluate(store, exec, "keys(o)");
    ASSERT_EQ(keys.kind(), Value::Kind::Array);
    const Store &made = owner(store, exec, keys);
    ...
    for (std::uint32_t i = 0; i < 3; ++i) {
        ...
        got.insert(std::string(owner(store, exec, key).string(key)));
    }
...
TEST(EvalCall, KeysOfEmptyObjectIsEmptyArray) {
    ...
    put(store, "o", "{}");
    const Value keys = evaluate(store, exec, "keys(o)");
    EXPECT_EQ(owner(store, exec, keys).arrayCount(keys), 0u);
}
...
TEST(EvalCall, KeysRejectsNonObjects) {
    ...
    CS::Execution exec;
    put(store, "items", "[1]");
    EXPECT_EQ(evalError(store, exec, "keys(items)").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "keys('a')").code, CS::ErrorCode::Type);
}
...
TEST(EvalCall, HasDistinguishesAbsentFromNull) {
    ...
    EXPECT_TRUE(evaluate(store, exec, "has(o, 'present')").booleanValue());
    EXPECT_TRUE(evaluate(store, exec, "has(o, 'empty')").booleanValue());
    EXPECT_FALSE(evaluate(store, exec, "has(o, 'missing')").booleanValue());
    EXPECT_EQ(evaluate(store, exec, "o.empty").kind(), Value::Kind::Null);
    EXPECT_EQ(evaluate(store, exec, "o.missing").kind(), Value::Kind::Null);
}
...
TEST(EvalCall, HasCoercesTheKey) {
    ...
    EXPECT_TRUE(evaluate(store, exec, "has(o, 0)").booleanValue());
    EXPECT_TRUE(evaluate(store, exec, "has(o, true)").booleanValue());
    EXPECT_EQ(evalError(store, exec, "has(o, [1])").code, CS::ErrorCode::Type);
}
...
TEST(EvalCall, LastOfArrayAndOfEmpty) {
    ...
    put(store, "items", "[10, 20, 30]");
    put(store, "empty", "[]");
    EXPECT_EQ(evaluate(store, exec, "last(items)").numberValue(), 30.0);
    // docs/semantics.md §8.4: на пустом — null, тогда как items[count-1] дал бы
    // ошибку. Обе половины в одном тесте.
    EXPECT_EQ(evaluate(store, exec, "last(empty)").kind(), Value::Kind::Null);
    EXPECT_EQ(evalError(store, exec, "empty[count(empty) - 1]").code,
              CS::ErrorCode::Range);
}
...
TEST(EvalCall, LastRejectsNonArrays) {
    ...
    CS::Execution exec;
    put(store, "o", "{}");
    EXPECT_EQ(evalError(store, exec, "last(o)").code, CS::ErrorCode::Type);
}
...
TEST(EvalCall, NestedCallsWork) {
    ...
    CS::Execution exec;
    put(store, "o", "{'a': 1, 'b': 2}");
    EXPECT_EQ(evaluate(store, exec, "count(keys(o))").numberValue(), 2.0);
}
...
TEST(EvalCall, ArgumentsAreEvaluatedLeftToRight) {
    ...
    EXPECT_EQ(evalError(store, exec, "has(items[-1], 2 + 'b')").code,
              CS::ErrorCode::Range);
}
...
TEST(EvalCall, VariadicFormatDoesNotOverflowTheArgumentBuffer) {
    ...
    EXPECT_EQ(evalText(store, exec, "format('${}${}${}${}${}', 1, 2, 3, 4, 5)"),
              "12345");
}
...
TEST(EvalCall, PushAppends) {
    ...
    CS::Execution exec;
    put(store, "items", "[1, 2]");
    run(store, exec, "push(items, 3);");
    EXPECT_EQ(evaluate(store, exec, "count(items)").numberValue(), 3.0);
    EXPECT_EQ(evaluate(store, exec, "items[2]").numberValue(), 3.0);
}
...
TEST(EvalCall, PushIsTheOnlyWayToGrowAnArray) {
    ...
    EXPECT_EQ(runError(store, exec, "items[1] = 2;").code, CS::ErrorCode::Range);
    run(store, exec, "push(items, 2);");
    EXPECT_EQ(evaluate(store, exec, "items[1]").numberValue(), 2.0);
}
...
TEST(EvalCall, PushRejectsNonArrays) {
    ...
    CS::Execution exec;
    put(store, "o", "{}");
    EXPECT_EQ(runError(store, exec, "push(o, 1);").code, CS::ErrorCode::Type);
}
...
TEST(EvalCall, PopRemovesAndDoesNothingOnEmpty) {
    ...
    put(store, "items", "[1, 2]");
    put(store, "empty", "[]");
    run(store, exec, "pop(items);");
    EXPECT_EQ(evaluate(store, exec, "count(items)").numberValue(), 1.0);
    // docs/semantics.md §8.6: на пустом ничего не делает и не отказывает.
    run(store, exec, "pop(empty);");
    EXPECT_EQ(evaluate(store, exec, "count(empty)").numberValue(), 0.0);
}
...
TEST(EvalCall, TakingTheRemovedElementNeedsTwoSteps) {
    ...
    run(store, exec, "state.taken = last(state.items); pop(state.items);");
    EXPECT_EQ(evaluate(store, exec, "state.taken").numberValue(), 3.0);
    EXPECT_EQ(evaluate(store, exec, "count(state.items)").numberValue(), 2.0);
}
...
TEST(EvalCall, PushAndPopAreVisibleThroughAnAlias) {
    ...
    run(store, exec, "push(state.items, 2);");
    EXPECT_EQ(evaluate(store, exec, "count(shortcut)").numberValue(), 2.0);
}
...
TEST(EvalCall, StrConvertsScalars) {
    ...
    EXPECT_EQ(evalText(store, exec, "str(1)"), "1");
    EXPECT_EQ(evalText(store, exec, "str(0.5)"), "0.5");
    EXPECT_EQ(evalText(store, exec, "str(true)"), "true");
    EXPECT_EQ(evalText(store, exec, "str(false)"), "false");
    EXPECT_EQ(evalText(store, exec, "str(null)"), "null");
    EXPECT_EQ(evalText(store, exec, "str('уже строка')"), "уже строка");
}
...
TEST(EvalCall, StrRejectsAggregates) {
    ...
    put(store, "items", "[1]");
    put(store, "o", "{}");
    EXPECT_EQ(evalError(store, exec, "str(items)").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "str(o)").code, CS::ErrorCode::Type);
}

TEST(EvalCall, MinAndMaxTakeExactlyTwo) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evaluate(store, exec, "min(1, 2)").numberValue(), 1.0);
    EXPECT_EQ(evaluate(store, exec, "max(1, 2)").numberValue(), 2.0);
    EXPECT_EQ(evaluate(store, exec, "min(-1, -2)").numberValue(), -2.0);
    // Для трёх и более — вложение (§8.9).
    EXPECT_EQ(evaluate(store, exec, "min(3, min(1, 2))").numberValue(), 1.0);
}

TEST(EvalCall, MinAndMaxRejectNonNumbers) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evalError(store, exec, "min('a', 1)").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "max(1, true)").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "min(null, 1)").code, CS::ErrorCode::Type);
}
...
TEST(EvalCall, MinAndMaxPropagateNaN) {
    ...
    EXPECT_TRUE(std::isnan(evaluate(store, exec, "min(0 / 0, 5)").numberValue()));
    EXPECT_TRUE(std::isnan(evaluate(store, exec, "min(5, 0 / 0)").numberValue()));
    EXPECT_TRUE(std::isnan(evaluate(store, exec, "max(0 / 0, 5)").numberValue()));
    EXPECT_TRUE(std::isnan(evaluate(store, exec, "max(5, 0 / 0)").numberValue()));
    // А бесконечность по-прежнему проходит насквозь, не превращаясь в NaN.
    EXPECT_EQ(evaluate(store, exec, "min(1, 1 / 0)").numberValue(), 1.0);
}

TEST(EvalCall, AbsIsTheModulus) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evaluate(store, exec, "abs(3)").numberValue(), 3.0);
    EXPECT_EQ(evaluate(store, exec, "abs(-3)").numberValue(), 3.0);
    EXPECT_EQ(evaluate(store, exec, "abs(0)").numberValue(), 0.0);
    EXPECT_EQ(evalError(store, exec, "abs('a')").code, CS::ErrorCode::Type);
}
...
TEST(EvalCall, RoundGoesAwayFromZero) {
    ...
    EXPECT_EQ(evaluate(store, exec, "round(0.5)").numberValue(), 1.0);
    EXPECT_EQ(evaluate(store, exec, "round(1.5)").numberValue(), 2.0);
    EXPECT_EQ(evaluate(store, exec, "round(2.5)").numberValue(), 3.0);
    EXPECT_EQ(evaluate(store, exec, "round(-0.5)").numberValue(), -1.0);
    EXPECT_EQ(evaluate(store, exec, "round(1.4)").numberValue(), 1.0);
    EXPECT_EQ(evalError(store, exec, "round(true)").code, CS::ErrorCode::Type);
}
...
TEST(EvalCall, ArithmeticBuiltinsFollowIEEEOnSpecialValues) {
    ...
    EXPECT_TRUE(std::isinf(evaluate(store, exec, "abs(inf)").numberValue()));
    EXPECT_TRUE(std::isinf(evaluate(store, exec, "max(1, inf)").numberValue()));
    EXPECT_EQ(evaluate(store, exec, "min(1, inf)").numberValue(), 1.0);
}
...
TEST(EvalFormat, SubstitutesLeftToRight) {
    ...
    EXPECT_EQ(evalText(store, exec, "format('Привет, ${}!', user.name)"),
              "Привет, Вася!");
    EXPECT_EQ(evalText(store, exec,
...
TEST(EvalFormat, EscapedPlaceholderIsLiteral) {
    ...
    EXPECT_EQ(evalText(store, exec, "format('цена $${}')"), "цена ${}");
    EXPECT_EQ(evalText(store, exec, "format('$${} и ${}', 1)"), "${} и 1");
}
...
TEST(EvalFormat, DollarWithoutBracesIsOrdinaryText) {
    ...
    EXPECT_EQ(evalText(store, exec, "format('a$b${}', 1)"), "a$b1");
    EXPECT_EQ(evalText(store, exec, "format('цена: 100$')"), "цена: 100$");
    EXPECT_EQ(evalText(store, exec, "format('$')"), "$");
    EXPECT_EQ(evalText(store, exec, "format('${')"), "${");
    EXPECT_EQ(evalText(store, exec, "format('$}')"), "$}");
    EXPECT_EQ(evalText(store, exec, "format('{}')"), "{}");
}
...
TEST(EvalFormat, MarkerIsFoundAfterARunOfDollars) {
    ...
    EXPECT_EQ(evalText(store, exec, "format('$$$${}${}', 1)"), "$$${}1");
}

TEST(EvalFormat, NoPlaceholdersGivesTheTemplate) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evalText(store, exec, "format('без подстановок')"),
              "без подстановок");
    EXPECT_EQ(evalText(store, exec, "format('')"), "");
}
...
TEST(EvalFormat, ArgumentsAreCoercedByChapterFour) {
    Store store;
    CS::Execution exec;
    EXPECT_EQ(evalText(store, exec, "format('${}', 0.5)"), "0.5");
    EXPECT_EQ(evalText(store, exec, "format('${}', true)"), "true");
    EXPECT_EQ(evalText(store, exec, "format('${}', null)"), "null");
}
...
TEST(EvalFormat, AggregateArgumentIsAnError) {
    ...
    CS::Execution exec;
    put(store, "items", "[1]");
    EXPECT_EQ(evalError(store, exec, "format('${}', items)").code, CS::ErrorCode::Type);
}
...
TEST(EvalFormat, NonStringTemplateIsAnError) {
    ...
    CS::Execution exec;
    put(store, "n", "1");
    EXPECT_EQ(evalError(store, exec, "format(n, 1)").code, CS::ErrorCode::Type);
}
...
TEST(EvalFormat, MismatchWithANonLiteralTemplateIsARuntimeError) {
    ...
    EXPECT_EQ(evalError(store, exec, "format(tpl.two, 1)").code, CS::ErrorCode::Type);
    EXPECT_EQ(evalError(store, exec, "format(tpl.none, 1)").code, CS::ErrorCode::Type);
    // А совпадающее число проходит.
    EXPECT_EQ(evalText(store, exec, "format(tpl.two, 1, 2)"), "1 и 2");
}
...
TEST(EvalFormat, ResultIsAnOrdinaryString) {
    ...
    run(store, exec, "state.label = format('${} шт.', 3);");
    EXPECT_EQ(evalText(store, exec, "state.label"), "3 шт.");
    // count у строки считает байты, а не символы (§8.1): пять символов
    // «3 шт.» дают семь байт, потому что ш и т по два.
    EXPECT_EQ(evaluate(store, exec, "count(state.label)").numberValue(), 7.0);
}
...
TEST(EvalFormat, LongResultCrossesPoolGrowth) {
    ...
    EXPECT_EQ(owner(store, exec, built).string(built), expected);
}
...
TEST(EvalFormat, ArgumentThatWritesToThePoolDoesNotLeakIntoTheResult) {
    ...
    EXPECT_EQ(evalText(store, exec, "format('Привет, ${}!', 'мир')"),
              "Привет, мир!");
    // Вложенная сборка не поглощается внешней.
    EXPECT_EQ(evalText(store, exec, "format('${}', format('${}', 1))"), "1");
    ...
    EXPECT_EQ(evalText(store, exec, "format('${}', str(2))"), "2");
}
...
TEST(EvalFormat, ResultSurvivesIntoTheData) {
    ...
    CS::Execution exec;
    put(store, "state", "{'label': ''}");
    run(store, exec, "state.label = format('${} шт.', 'десять');");
    EXPECT_EQ(evalText(store, exec, "state.label"), "десять шт.");
}
...
TEST(EvalFormat, NestedAssemblyKeepsTheOuterPrefix) {
    ...
    EXPECT_EQ(evalText(store, exec, "format('a${}b', format('c${}d', 1))"),
              "ac1db");
    ...
    EXPECT_EQ(
        evalText(store, exec, "format('1${}2', format('3${}4', format('5${}6', 7)))"),
        "1357642");
}
