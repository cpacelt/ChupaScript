...
extension String: CSValue {
    ...
    /// Байты движок отдаёт срезом без владения и без копии — они лежат в
    /// `Context::lastResult_`, где может быть как строка, уложенная внутри
    /// самого значения, так и отдельно выделенный box; возвращаемая строка
    /// копирует байты себе, и это единственная копия на весь путь.
    ///
