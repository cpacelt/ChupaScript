...
/// Границу операции открывает только Context: на ней сливается список
/// отложенного освобождения (execution.hpp). Замер мимо него мерил бы
/// вычисление без той работы, которую хост оплачивает на каждом вызове, и
/// вдобавок копил бы в списке всё, что породили миллионы итераций, — то есть
/// измерял бы установившийся режим, в который движок в этой сборке не входит
/// никогда.
...
/// решение, меняющее раскладку Ast::Node или порядок веток в eval, отражается здесь
