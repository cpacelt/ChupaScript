...
class Store {
   ...
   private:
    ...
    /// не адресует; ссылающееся — только своего региона. Чужое сюда попасть не
    /// должно: вычислитель обязан провести его через promote, иначе записанный
    /// индекс переживёт сброс региона, на который указывал.
    [[nodiscard]] bool writable(Value v) const noexcept {
        return !v.addressesStore() || sameRegion(v);
    }
