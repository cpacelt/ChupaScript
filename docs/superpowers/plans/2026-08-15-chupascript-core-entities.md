...
- Стандарт — **C++17** (`CMakeLists.txt:15`), а не C++20. Ни одна задача плана C++20-фич не требует; если покажется, что требует, — это повод остановиться, а не поднять стандарт.
- `-Wextra` включён всегда, но `-Werror` — **нет**: `CHUPASCRIPT_WERROR` по умолчанию `OFF` (`CMakeLists.txt:46`). Предупреждения сборку не роняют, поэтому их надо ловить глазами.
- **Сборка с `-DCHUPASCRIPT_WERROR=ON` сегодня не проходит, и не по вине этого плана.** `core/include/chupascript/chupascript.h` пользуется `_Nullable` — это расширение Clang, и `-Wnullability-extension` роняет на нём сборку. Расширение стоит там намеренно, ради Swift. Пока это не снято, проверяй свои новые файлы под `-Werror` поштучно, а не сборкой целиком. Снимает это задача 5 — она же последняя, кто правит заголовок.
...
- Конструктор `Expression`/`Script` — публичный (иначе не соберутся ни обёртки C API, ни тесты). Инвариант держат приватные `source_`/`ast_` без сеттеров: наполненную единицу создаёт только `compile`. Пустая единица годна лишь под `compile`, `eval`/`run` на ней падают утверждением.
