...
TEST(PrintValue, Scalars) {
    Store store;
    EXPECT_EQ(chupa::printValue(store, Value::null()), "null");
    EXPECT_EQ(chupa::printValue(store, Value::boolean(true)), "true");
    EXPECT_EQ(chupa::printValue(store, Value::boolean(false)), "false");
    EXPECT_EQ(chupa::printValue(store, Value::number(42)), "42");
    EXPECT_EQ(chupa::printValue(store, Value::number(0.5)), "0.5");
    EXPECT_EQ(chupa::printValue(store, Value::number(-0.0)), "-0");
}
...
TEST(PrintValue, StringsAreQuoted) {
    ...
    EXPECT_EQ(chupa::printValue(store, put(store, "s", "'привет'")), "'привет'");
    EXPECT_EQ(chupa::printValue(store, put(store, "e", "''")), "''");
}
...
TEST(PrintValue, StringsAreEscapedBackToSource) {
    ...
    EXPECT_EQ(chupa::printValue(store, put(store, "q", "'it\\'s'")), "'it\\'s'");
    EXPECT_EQ(chupa::printValue(store, put(store, "b", "'a\\\\b'")), "'a\\\\b'");
    EXPECT_EQ(chupa::printValue(store, put(store, "n", "'a\\nb'")), "'a\\nb'");
    EXPECT_EQ(chupa::printValue(store, put(store, "t", "'a\\tb'")), "'a\\tb'");
}

TEST(PrintValue, EmptyAggregates) {
    Store store;
    EXPECT_EQ(chupa::printValue(store, put(store, "a", "[]")), "[]");
    EXPECT_EQ(chupa::printValue(store, put(store, "o", "{}")), "{}");
}

TEST(PrintValue, ArraysAndObjects) {
    Store store;
    EXPECT_EQ(chupa::printValue(store, put(store, "a", "[1, 2, 3]")), "[1, 2, 3]");
    EXPECT_EQ(chupa::printValue(store, put(store, "o", "{'a': 1}")), "{'a': 1}");
}
...
TEST(PrintValue, NestedAggregates) {
    ...
    EXPECT_EQ(chupa::printValue(store, v),
...
TEST(PrintValue, SelfReferencingObjectTerminates) {
    ...
    EXPECT_EQ(chupa::printValue(store, o), "{'n': 1, 'self': {...}}");
}
...
TEST(PrintValue, SelfReferencingArrayTerminates) {
    ...
    store.arrayPush(a, a);
    EXPECT_EQ(chupa::printValue(store, a), "[1, [...]]");
}
...
TEST(PrintValue, SharedAggregateIsPrintedInFullTwice) {
    ...
    EXPECT_EQ(chupa::printValue(store, holder), "{'a': [1, 2], 'b': [1, 2]}");
}
...
TEST(PrintValue, DeepNonCyclicTreeIsTruncated) {
    ...
    const std::string printed = chupa::printValue(store, arrays[0]);
...
TEST(PrintValue, MutualCycleTerminates) {
    ...
    EXPECT_EQ(chupa::printValue(store, a), "{'b': {'a': {...}}}");
}
