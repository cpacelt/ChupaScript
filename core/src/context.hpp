...
class Context {
   public:
    ...
    // format, ключ объекта) само пишет в text_ и не спрашивает разрешения.
    ...
    /// Заводит корень либо заменяет значение существующего. Имя не
    /// проверяется: требование «всякий корень адресуем идентификатором»
    /// держится на вызывающем, и единственный такой вызывающий —
    /// setVariable из data.hpp.
   ...
   private:
    ...
    // roots_ объявлено после пулов совершенно сознательно: конструктор
    // присваивает его в теле (`roots_ = makeObject();`), а не в списке
    ...
    // а не списком инициализации, поэтому останься roots_ выше пулов — тело
    // конструктора продолжало бы работать (присваивание случается уже после
    // того, как все члены построены), но конструктора с roots_ в списке
