...
/// держит их ссылку в pending_, а дальше судьбу коробки решают те, кто его
/// удержал — ячейка глобальной переменной, другая коробка либо хост.
///
...
class Store {
   public:
    ...
    /// временного региона — такая же коробка и умеет уехать.
   ...
   private:
    ...
    /// Собственно слив. Отделён от проверки, чтобы та встраивалась.
    void drainPendingSlow() noexcept;

    /// Собственно сброс арены — по той же причине, что и слив.
    ...
    std::vector<detail::Box *> pending_;
