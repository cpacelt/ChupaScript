...
/// Один "кадр записи" устройства потребителя. Два вида — не стилистика, а
/// следствие языка (docs/semantics.md §7.2): присвоить самому имени
/// глобальной переменной скрипт не может (`button_enabled = true;` — ошибка
/// компиляции, "cannot assign to a variable name", core/src/check.cpp:132),
/// только его полю или элементу. Поэтому скаляр меняет хост напрямую —
/// Context::setGlobal с готовым Value, тем же типизированным путём, каким
/// реальный BDUI пушит новые props (а не setVariableText: тот заново гоняет
/// значение через разбор текста, и мерил бы стоимость парсера, а не запись),
/// — а путь внутрь объекта меняет скрипт, тем же путём, каким его меняет
/// обработчик. Оба вида бьют по эпохе, которую видит evalTracked; разница
/// только в том, чья дверь записи это делает.
class Writer {
   public:
